      ...
      if (!error && data) {
        ...
        container.innerHTML = data.map(o => `
          <div class="history-item" onclick="showTrackPage('${o.order_number || o.id}')">
            <div class="h-info">
              <h4>${o.item}</h4>
              <p>${new Date(o.created_at).toLocaleDateString()} â€¢ ${o.fee}</p>
            </div>
    ...
    async function tncAgree() {
      ...
      await loadLiveFee();
      const e1 = document.getElementById('txt-inside-fee'); if (e1) e1.textContent = `Fixed fee â€” â‚¦${INSIDE_FEE}`;
      const e2 = document.getElementById('fo-txt-inside-fee'); if (e2) e2.textContent = `Fixed fee â€” â‚¦${INSIDE_FEE}`;
      const e3 = document.getElementById('er-txt-inside-fee'); if (e3) e3.textContent = `Fixed fee â€” â‚¦${INSIDE_FEE}`;
    ...
    function addFoItem() {
      ...
      row.innerHTML = `
        ...
        <input type="number" class="fo-item-price" placeholder="Price (â‚¦)" style="flex:1" min="0" oninput="calcFoTotal()" />
        <button onclick="this.parentElement.remove();calcFoTotal()" style="background:var(--red-dim);border:1px solid var(--red-border);color:var(--red);border-radius:6px;padding:0 10px;cursor:pointer;font-size:16px">âœ•</button>
      `;
    ...
    function calcFoTotal() {
      ...
      document.getElementById('fo-total-display').textContent = `Items Total: â‚¦${foItemsTotal}`;
    }
    ...
    async function submitFoodOrder() {
      ...
      btn.textContent = 'SAVING ORDERâ€¦'; btn.disabled = true;
      await loadLiveFee();
      ...
      names.forEach((n, i) => {
        if (n.value.trim() && qtys[i]?.value > 0) {
          itemsArr.push(`${n.value.trim()} x${qtys[i].value} @ â‚¦${prices[i]?.value || 0}`);
        }
      });
      calcFoTotal();
      const itemDesc = `[FOOD from ${eateryName}] ${itemsArr.join(', ')} | Items Total: â‚¦${foItemsTotal}`;
      const fee = foDeliveryType === 'inside' ? `â‚¦${INSIDE_FEE + foItemsTotal}` : 'Via WhatsApp';
      ...
      try {
        ...
        btn.textContent = 'REVIEW ORDER â†’'; btn.disabled = false;
      }
    }
    ...
    async function submitErrand() {
      ...
      btn.textContent = 'SAVING ORDERâ€¦'; btn.disabled = true;
      await loadLiveFee();
      const fee = erDeliveryType === 'inside' ? `â‚¦${INSIDE_FEE}` : 'Via WhatsApp';
      const ordNum = String(Math.floor(100000 + Math.random() * 900000));
    ...
    function addFoodItem() {
      ...
      row.innerHTML = `
        ...
        <input type="number" class="f-food-price" placeholder="Price (â‚¦)" style="flex:1" min="0" oninput="calcFoodTotal()" />
    ...
    function calcFoodTotal() {
      ...
      document.getElementById('food-total-display').textContent = 'Items Total: â‚¦' + foodItemsTotal;
    }
    ...
    async function submitOrder() {
      ...
      if (!_supaClient) {
        ...
        if (btn) { btn.disabled = false; btn.textContent = 'REVIEW ORDER â†’'; }
        return;
      }
      ...
      const orderData = {
        ...
        type: deliveryType,
        fee: deliveryType === 'inside' ? `â‚¦${INSIDE_FEE}` : 'Via WhatsApp',
        time: timeStr,
        status: 'pending',
    ...
    async function goToPayment() {
      ...
      if (deliveryType === 'inside' || hasNumericFee) {
        ...
        document.getElementById('pay-amount').textContent = `â‚¦${totalAmount}`;
        document.getElementById('btn-pay-now').style.display = '';
        document.getElementById('btn-pay-now').textContent = ` PAY â‚¦${totalAmount} SECURELY`;
        document.getElementById('pay-verifying').style.display = 'none';
      } else {
        // Outside campus â€” show WhatsApp card
    ...
    function launchPaystack() {
      ...
      btn.disabled = true;
      btn.textContent = 'Opening paymentâ€¦';
      ...
      const handler = PaystackPop.setup({
        ...
        callback: function (response) {
          // Payment popup closed with success â€” now verify server-side
    ...
    async function verifyAndConfirmPayment(reference) {
      try {
        ...
        if (res.ok && result.verified) {
          // âœ… Payment confirmed â€” show success receipt
          ...
          // âŒ Verification failed â€” show failed receipt
          document.getElementById('pay-verifying').style.display = 'none';
          const btn = document.getElementById('btn-pay-now');
          btn.style.display = '';
          btn.disabled = false;
    ...
    function renderTrackResult(order) {
      ...
      const badgeLabels = { pending: 'Awaiting Payment', paid: 'Paid â€” Preparing', assigned: 'Rider Dispatched', delivered: 'Delivered âœ“' };
      ...
      steps.forEach((s, i) => {
        ...
        if (i < idx) {
          dot.classList.add('done'); dot.textContent = 'âœ“';
      ...
      // Details
      document.getElementById('tr-name').textContent = order.name || 'â€”';
      document.getElementById('tr-item').textContent = order.item || 'â€”';
      document.getElementById('tr-pickup').textContent = order.pickup || 'â€”';
      document.getElementById('tr-dropoff').textContent = order.dropoff || 'â€”';
      document.getElementById('tr-fee').textContent = order.fee || 'â€”';
    ...
    async function submitReview() {
      ...
      try {
        ...
        const stars = 'â˜…'.repeat(_reviewStar) + 'â˜†'.repeat(5 - _reviewStar);
        ...
        card.innerHTML = `
          ...
          <div class="rev-author">
            ...
            <div>
              ...
              <div class="r-sub">Student â€¢ LAUTECH</div>
            </div>
          </div>`;
        ...
        console.error(e);
        closeReviewModal();
        alert('Review submitted â€” thanks for the feedback!');
      }
